...
from anysat.models.networks.encoder.utils.pos_embed import (
...
class AnyModule(nn.Module):
    ...
    def __init__(
        ...
        if not release:
            ...
            for dataset in self.datasets:
                for scale in scales[dataset]:
                    ...
                    self.pos_embed["_".join([dataset, str(scale)])] = (
                        get_2d_sincos_pos_embed_with_scale(
                            embed_dim, int(num_p**0.5), scale, cls_token=class_token
                        )
                    )
        ...
        dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, depth + 1)
        ...
        for modality in modalities:
            ...
            if pos_embed is None and modality != "modis":
                ...
                pos_embed = get_2d_sincos_pos_embed_with_scale(
                    C, num_patches, scale, cls_token=True
                ).to(device)
